...
BiTree CreateBiTree2(int a[],int low1,int low2,int hig1,int hig2)
{
	...
	int m=low2;
	while(a[low1]==m+1) m++;
	p->lchild=Create(a,low1+1,low1+m-low2,low2,m-1);
	p->rchild=Create(a,low1+m-low2+1,hig1,m+1,hig2);
	return p;
}
